...
export default function ServicesPage() {
  ...
  const handleServiceClick = (svc) => {
    // Research Projects gets a special sub-menu instead of the inline form
    if (svc.path === '/services/research-projects') {
      setShowResearchMenu(true);
      setSelectedService(null);
      setTimeout(() => {
        document.getElementById('research-menu')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }, 80);
      return;
    }
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.container}>
        ...
        {/* Research Sub-Menu */}
        {showResearchMenu && !selectedService && !submitted && (
          <div id="research-menu" style={S.formCard}>
            <button
              type="button"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '20px 0 8px' }}>
              ...
              <h2 style={S.formTitle}>Research Projects</h2>
            </div>
            <p style={S.formSubtitle}>
              Complete research assistance from topic selection to final write-up. How would you like to proceed?
            </p>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 8 }}>
              <button
                onClick={() => navigate('/topics')}
                style={S.researchOptionBtn}
                onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.18)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.5)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.08)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.25)'; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
                <span style={S.researchOptionIcon}>📚</span>
                <div style={S.researchOptionText}>
                  <span style={S.researchOptionTitle}>Browse Research Topics</span>
                  <span style={S.researchOptionDesc}>Explore our library of pre-researched topics ready to be written to your specifications.</span>
                </div>
              ...
              <button
                onClick={() => {
                  const researchSvc = SERVICES.find(s => s.path === '/services/research-projects');
                  setShowResearchMenu(false);
                  setSelectedService(researchSvc);
                  const init = { details: '', deadline: '' };
                  researchSvc.fields.forEach(f => { init[f.name] = ''; });
                  setForm(init);
                  setTimeout(() => {
                    document.getElementById('request-form')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
                  }, 80);
                ...
                onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.08)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.25)'; e.currentTarget.style.transform = 'translateY(0)'; }}
              >
                <span style={S.researchOptionIcon}>✍️</span>
                <div style={S.researchOptionText}>
                  <span style={S.researchOptionTitle}>Request a Research</span>
                  <span style={S.researchOptionDesc}>Submit your own topic — we handle literature reviews, methodology, data collection, and full project documentation.</span>
                </div>
                <span style={{ color: 'rgba(255,255,255,0.5)', fontSize: 20, flexShrink: 0 }}>→</span>
              </button>
            </div>
        ...
        {selectedService && !submitted && (
          <div id="request-form" style={S.formCard}>
            <button
              type="button"
              onClick={() => {
                if (selectedService?.path === '/services/research-projects') {
                  setSelectedService(null);
                  setShowResearchMenu(true);
                  setTimeout(() => document.getElementById('research-menu')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 80);
                } else {
                  setSelectedService(null);
                }
              }}
...
const S = {
  ...
  researchOptionBtn:  { display: 'flex', alignItems: 'center', gap: 16, background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.25)', borderRadius: 12, padding: '18px 20px', cursor: 'pointer', transition: 'all 0.2s ease', textAlign: 'left', width: '100%' },
  researchOptionIcon: { fontSize: 28, flexShrink: 0 },
  researchOptionText: { flex: 1, display: 'flex', flexDirection: 'column', gap: 4 },
  researchOptionTitle:{ color: '#fff', fontSize: 15, fontWeight: 700, fontFamily: "'Times New Roman',Georgia,serif", letterSpacing: '0.03em' },
  researchOptionDesc: { color: 'rgba(255,255,255,0.6)', fontSize: 12, lineHeight: 1.5, fontFamily: "'Times New Roman',Georgia,serif" },
};
