...
export default function RegisterPage() {
  ...
  const refCode = searchParams.get('ref') || null; // writer's uid or 'admin'
  const password = watch('password', '');

  const onSubmit = async (data) => {
    try {
      await registerUser({ name: data.name, email: data.email, password: data.password, role, referredBy: refCode });
