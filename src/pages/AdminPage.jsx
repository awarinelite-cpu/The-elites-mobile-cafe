...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', paddingTop: 64, position: 'relative' }}>
          ...
          <div className="req-grid" style={{ display: 'grid', gridTemplateColumns: selectedRequest ? '1fr 1fr' : '1fr', gap: 20 }}>
            ...
            {selectedRequest && (
              <div className="req-detail-panel" style={{ ...s.card, position: 'sticky', top: 80, maxHeight: 'calc(100vh - 120px)', overflowY: 'auto' }}>
                ...
                {(() => {
                  const pendingReceipts = orderChatMessages.filter(
                    m => m.type === 'payment_receipt' &&
                    selectedRequest?.paymentStatus !== `${m.paymentStage}_confirmed`
                  );
                  if (pendingReceipts.length === 0) return null;
                  // Show the most recent unconfirmed receipt
                  const receipt = pendingReceipts[pendingReceipts.length - 1];
                  const isImage = receipt.fileType?.startsWith('image/');
                  return (
                    <div style={{ marginTop: 16, background: 'rgba(245,158,11,0.07)', border: '1.5px solid rgba(245,158,11,0.45)', borderRadius: 12, overflow: 'hidden' }}>
                      <div style={{ padding: '10px 14px', background: 'rgba(245,158,11,0.13)', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ fontSize: 18 }}>🧾</span>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: 13, color: '#D97706' }}>
                            Payment Receipt — {receipt.stageLabel}
                          </div>
                          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 1 }}>
                            From {receipt.senderName} · awaiting your confirmation
                          </div>
                        </div>
                      </div>
                      {/* Image preview — click to enlarge */}
                      {isImage && receipt.fileData && (
                        <img src={receipt.fileData} alt="Receipt"
                          onClick={() => setLightboxImage(receipt.fileData)}
                          style={{ width: '100%', maxHeight: 200, objectFit: 'contain', display: 'block', cursor: 'zoom-in', borderBottom: '1px solid var(--border)' }} />
                      )}
                      <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontSize: 16 }}>{isImage ? '🖼️' : '📄'}</span>
                          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', flex: 1, wordBreak: 'break-word' }}>{receipt.fileName}</span>
                          {isImage ? (
                            <button onClick={() => setLightboxImage(receipt.fileData)}
                              style={{ fontSize: 11, color: 'var(--teal)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                              👁️ View Full
                            </button>
                          ) : (
                            <a href={receipt.fileData} download={receipt.fileName}
                              style={{ fontSize: 11, color: 'var(--teal)', fontWeight: 700, textDecoration: 'none' }}>
                              ⬇️ Download
                            </a>
                          )}
                        </div>
                        <button
                          ...
                          style={{ width: '100%', background: '#16A34A', color: '#fff', border: 'none', borderRadius: 8, padding: '11px', cursor: confirmingPayment ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: 14, fontFamily: 'var(--font-body)', opacity: confirmingPayment ? 0.6 : 1, marginTop: 2 }}>
                          {confirmingPayment ? '⏳ Confirming...' : `✅ Confirm ${receipt.paymentStage === 'advance' ? 'Advance' : 'Final'} Payment`}
                        </button>
                      </div>
                    </div>
                  );
                              ...
                              <div style={{ width: '100%', maxWidth: 360, borderRadius: 14, overflow: 'hidden', border: `1.5px solid ${isConfirmed ? 'rgba(22,163,74,0.45)' : 'rgba(245,158,11,0.55)'}`, background: 'var(--bg-card)', boxShadow: '0 2px 12px rgba(0,0,0,0.12)' }}>
                                ...
                                {isImage && m.fileData && (
                                  <img src={m.fileData} alt="Receipt"
                                    onClick={() => setLightboxImage(m.fileData)}
                                    style={{ width: '100%', maxHeight: 220, objectFit: 'contain', display: 'block', borderBottom: '1px solid var(--border)', cursor: 'zoom-in' }} />
                                )}
                                <div style={{ padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    ...
                                    {isImage ? (
                                      <button onClick={() => setLightboxImage(m.fileData)}
                                        style={{ fontSize: 11, color: 'var(--teal)', fontWeight: 700, background: 'none', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap', padding: 0 }}>
                                        👁️ View
                                      </button>
                                    ) : (
      ...
      {/* Lightbox — full-size receipt viewer */}
      {lightboxImage && (
        <div onClick={() => setLightboxImage(null)}
          style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'zoom-out', padding: 20 }}>
          <img src={lightboxImage} alt="Receipt full view"
            style={{ maxWidth: '100%', maxHeight: '90vh', objectFit: 'contain', borderRadius: 10, boxShadow: '0 8px 40px rgba(0,0,0,0.6)' }} />
          <button onClick={() => setLightboxImage(null)}
            style={{ position: 'absolute', top: 18, right: 22, background: 'rgba(255,255,255,0.15)', border: 'none', color: '#fff', borderRadius: '50%', width: 38, height: 38, fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>✕</button>
        </div>
      )}
