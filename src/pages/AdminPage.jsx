...
const NAV = [
  ...
  { key: 'receipts',   label: '🧾 Payment Receipts',  group: 'main'    },
...
export default function AdminPage() {
  ...
  const [receiptNotifications, setReceiptNotifications] = useState([]);
  const [viewingReceipt, setViewingReceipt]   = useState(null); // notification doc
  ...
  // ── Realtime: payment receipt notifications from clients ─
  useEffect(() => {
    const unsub = onSnapshot(
      query(
      ...
      snap => setReceiptNotifications(snap.docs.map(d => ({ id: d.id, ...d.data() })))
    );
    return unsub;
  }, []);
  ...
  // ── Confirm Manual Payment (from notification receipt) ──
  const confirmManualPayment = async (stage, notificationId, targetOrderId, targetUserId, orderTitle) => {
    ...
    try {
      ...
      // 2. Mark the notification as confirmed
      ...
      // 4. Update local selectedRequest if open
  ...
  const navBadge = (key) => {
    ...
    if (key === 'receipts') {
      const unconfirmed = receiptNotifications.filter(n => !n.confirmed).length;
      return unconfirmed > 0 ? unconfirmed : null;
    }
    return null;
  };
      ...
      <aside className={`admin-sidebar${sidebarOpen ? '' : ' closed'}`}
        ...
        <div style={{ padding: '20px 18px 16px', borderBottom: '1px solid var(--border)', background: 'linear-gradient(135deg,var(--blue-deep),var(--teal-dark))' }}>
          ...
          <div style={{ marginTop: 10, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            {receiptNotifications.filter(n => !n.confirmed).length > 0 && (
              <span style={{ background: 'rgba(13,148,136,0.85)', color: '#fff', borderRadius: 20, padding: '2px 9px', fontSize: 11, fontWeight: 700 }}>
                🧾 {receiptNotifications.filter(n => !n.confirmed).length} receipt{receiptNotifications.filter(n => !n.confirmed).length > 1 ? 's' : ''}
              </span>
            )}
          </div>
        </div>
        ...
        {tab === 'receipts' && (
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 24 }}>
              Clients send their transfer receipts here. Review each one and confirm to unlock their order.
            </p>

            {receiptNotifications.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px 24px', ...s.card }}>
                <div style={{ fontSize: '3rem', marginBottom: 12 }}>🧾</div>
                <div style={{ fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}>No receipts yet</div>
                <div style={{ color: 'var(--text-muted)', fontSize: 13 }}>When clients send payment receipts they will appear here.</div>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                {receiptNotifications.map(notif => {
                  const isConfirmed = notif.confirmed;
                  const isImage     = notif.receiptType?.startsWith('image/');
                  const stageLabel  = notif.paymentStage === 'advance' ? '50% Advance' : '50% Final Balance';
                  return (
                    <div key={notif.id} style={{
                      ...s.card, padding: 0, overflow: 'hidden',
                      border: `1.5px solid ${isConfirmed ? 'rgba(22,163,74,0.35)' : 'rgba(245,158,11,0.45)'}`,
                    }}>
                      {/* Header strip */}
                      <div style={{
                        padding: '12px 16px',
                        background: isConfirmed ? 'rgba(22,163,74,0.10)' : 'rgba(245,158,11,0.10)',
                        display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8,
                      }}>
                        <div>
                          <div style={{ fontWeight: 700, fontSize: 14, color: isConfirmed ? '#16A34A' : '#D97706' }}>
                            {isConfirmed ? '✅' : '⏳'} {stageLabel} Receipt — {notif.clientName}
                          </div>
                          <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>
                            {notif.orderTitle} · {notif.createdAt?.toDate?.()?.toLocaleString?.() || 'Just now'}
                          </div>
                        </div>
                        <span style={{
                          fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 20,
                          background: isConfirmed ? 'rgba(22,163,74,0.15)' : 'rgba(245,158,11,0.15)',
                      ...
                      <div style={{ padding: '14px 16px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                          <span style={{ fontSize: 22 }}>{isImage ? '🖼️' : '📄'}</span>
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{notif.receiptName}</div>
                            <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                              {notif.receiptSize ? `${(notif.receiptSize / 1024).toFixed(1)} KB` : ''} · {notif.receiptType}
                            </div>
                          </div>
                          <a href={notif.receiptData} download={notif.receiptName}
                            style={{ marginLeft: 'auto', ...s.btn('var(--teal)'), padding: '6px 14px', fontSize: 12, textDecoration: 'none' }}>
                            ⬇️ Download
                          </a>
                        </div>

                        {/* Image preview */}
                        {isImage && notif.receiptData && (
                          <img src={notif.receiptData} alt="Receipt"
                            style={{ width: '100%', maxHeight: 280, objectFit: 'contain', borderRadius: 10, border: '1px solid var(--border)', marginBottom: 14 }} />
                        )}

                        {/* Non-image preview hint */}
                        {!isImage && (
                          <div style={{ background: 'rgba(13,148,136,0.06)', border: '1px solid rgba(13,148,136,0.15)', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 13, color: 'var(--text-muted)' }}>
                            📎 Non-image file — click Download to open and verify the receipt.
                          </div>
                        )}
                        ...
                        {!isConfirmed ? (
                          <button
                            ...
                            style={{ width: '100%', background: '#16A34A', color: '#fff', border: 'none', borderRadius: 10, padding: '13px', cursor: 'pointer', fontWeight: 700, fontSize: 14, fontFamily: 'var(--font-body)', opacity: confirmingPayment ? 0.6 : 1 }}>
                            {confirmingPayment ? '⏳ Confirming...' : `✅ Confirm ${notif.paymentStage === 'advance' ? 'Advance' : 'Final'} Payment`}
                          </button>
                        ) : (
                          <div style={{ textAlign: 'center', fontWeight: 700, color: '#16A34A', fontSize: 14, padding: '10px 0', background: 'rgba(22,163,74,0.07)', borderRadius: 8 }}>
                            ✅ Payment Confirmed — Client has been notified
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="req-grid" style={{ display: 'grid', gridTemplateColumns: selectedRequest ? '1fr 1fr' : '1fr', gap: 20 }}>
            ...
            {selectedRequest && (
              <div className="req-detail-panel" style={{ ...s.card, position: 'sticky', top: 80, maxHeight: 'calc(100vh - 120px)', overflowY: 'auto' }}>
                ...
                {(() => {
                  const orderReceipts = receiptNotifications.filter(n => n.orderId === selectedRequest.id);
                  ...
                  return (
                    <>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
                        Client has sent {orderReceipts.length} receipt{orderReceipts.length > 1 ? 's' : ''}. Review and confirm below.
                      </div>
                      {orderReceipts.map(notif => {
                        const isConfirmed = notif.confirmed;
                        const isImage     = notif.receiptType?.startsWith('image/');
                        const stageLabel  = notif.paymentStage === 'advance' ? '50% Advance' : '50% Final Balance';
                        return (
                          <div key={notif.id} style={{
                            ...
                            border: `1.5px solid ${isConfirmed ? 'rgba(22,163,74,0.35)' : 'rgba(245,158,11,0.40)'}`,
                          }}>
                            <div style={{
                              padding: '10px 14px',
                              background: isConfirmed ? 'rgba(22,163,74,0.10)' : 'rgba(245,158,11,0.10)',
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
                                  {isConfirmed ? '✅ Confirmed' : '⏳ Awaiting your confirmation'}
                                </div>
                              </div>
                            </div>
                            <div style={{ padding: '10px 14px', background: 'var(--bg-card)' }}>
                              {isImage && notif.receiptData && (
                                <img src={notif.receiptData} alt="Receipt"
                                  style={{ width: '100%', maxHeight: 200, objectFit: 'contain', borderRadius: 8, border: '1px solid var(--border)', marginBottom: 10 }} />
                              )}
                              {!isImage && (
                                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 10 }}>
                                  📎 {notif.receiptName} — <a href={notif.receiptData} download={notif.receiptName} style={{ color: 'var(--teal)', fontWeight: 600 }}>Download to view</a>
                                </div>
                              )}
                              {!isConfirmed && (
                                <button onClick={() => confirmManualPayment(
                                  notif.paymentStage, notif.id, notif.orderId, notif.clientId, notif.orderTitle
                                )}
                                  disabled={confirmingPayment || saving}
                                  style={{ width: '100%', background: '#16A34A', color: '#fff', border: 'none', borderRadius: 10, padding: '11px', cursor: 'pointer', fontWeight: 700, fontSize: 14, fontFamily: 'var(--font-body)', opacity: confirmingPayment ? 0.6 : 1 }}>
                                  {confirmingPayment ? '⏳ Confirming...' : `✅ Confirm ${notif.paymentStage === 'advance' ? 'Advance' : 'Final'} Payment`}
                                </button>
                              )}
                              {isConfirmed && (
                                <div style={{ textAlign: 'center', fontWeight: 700, color: '#16A34A', fontSize: 14, padding: '8px 0' }}>
                                  ✅ Payment Confirmed
                                </div>
                              )}
                            </div>
