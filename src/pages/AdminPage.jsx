...
export default function AdminPage() {
  ...
  const confirmManualPayment = async (stage) => {
    if (!selectedRequest) return;
    setConfirmingPayment(true);
    try {
      const receiptKey    = `manualReceipt_${stage}`;
      const newStatus     = `${stage}_confirmed`;
      const storedReceipt = selectedRequest[receiptKey] || {};

      await updateDoc(doc(db, 'serviceRequests', selectedRequest.id), {
        paymentStatus: newStatus,
        [`${receiptKey}.status`]: 'confirmed',
      ...
      setSelectedRequest(r => ({
        ...r,
        paymentStatus: newStatus,
        [receiptKey]: { ...storedReceipt, status: 'confirmed' },
      }));
      ...
      if (selectedRequest.userId) {
        await addDoc(collection(db, 'notifications'), {
          ...
          body:      stage === 'advance'
            ? `Your advance payment for "${selectedRequest.serviceTitle || 'your order'}" has been confirmed. Work will begin shortly.`
            : `Your final payment for "${selectedRequest.serviceTitle || 'your order'}" has been confirmed. You can now download your completed work.`,
          type:      'payment',
      ...
      showToast(`✅ ${stage} payment confirmed & client notified`);
    } catch (e) { showToast('❌ ' + e.message); }
            ...
            {selectedRequest && (
              <div className="req-detail-panel" style={{ ...s.card, position: 'sticky', top: 80, maxHeight: 'calc(100vh - 120px)', overflowY: 'auto' }}>
                ...
                {(() => {
                  ...
                  const hasAnyReceipt = stages.some(st => selectedRequest[`manualReceipt_${st}`]?.receiptData);
                  ...
                  return (
                    <>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 14 }}>
                        Client-uploaded payment receipts. Review and confirm below.
                      </div>
                      {stages.map(stage => {
                        const receiptKey  = `manualReceipt_${stage}`;
                        const receipt     = selectedRequest[receiptKey];
                        const payStatus   = selectedRequest.paymentStatus || '';
                        const isPending   = payStatus === `${stage}_receipt_pending`;
                        const isConfirmed = payStatus === `${stage}_confirmed`;
                        if (!receipt?.receiptData) return null;
                        return (
                          <div key={stage} style={{
                            borderRadius: 12,
                            border: `1.5px solid ${isConfirmed ? 'rgba(22,163,74,0.35)' : isPending ? 'rgba(245,158,11,0.4)' : 'var(--border)'}`,
                            overflow: 'hidden', marginBottom: 14,
                          }}>
                            <div style={{
                              padding: '10px 14px',
                              background: isConfirmed ? 'rgba(22,163,74,0.10)' : isPending ? 'rgba(245,158,11,0.10)' : 'var(--bg-tertiary)',
                              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                            }}>
                              <div>
                                <div style={{ fontWeight: 700, fontSize: 13, color: isConfirmed ? '#16A34A' : isPending ? '#D97706' : 'var(--text-primary)' }}>
                                  {stage === 'advance' ? '50% Advance' : '50% Final'} Receipt
                                </div>
                                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
                                  {isConfirmed ? '✅ Confirmed' : isPending ? '⏳ Awaiting your confirmation' : receipt.status || 'uploaded'}
                                </div>
                              </div>
                              {receipt.detectedAmount && (
                                <div style={{ background: 'rgba(13,148,136,0.12)', border: '1px solid rgba(13,148,136,0.3)', borderRadius: 20, padding: '4px 12px', fontSize: 13, fontWeight: 700, color: '#0D9488' }}>
                                  ₦{Number(receipt.detectedAmount).toLocaleString()}
                                </div>
                              )}
                            </div>
                            <div style={{ padding: '10px 14px', background: 'var(--bg-card)' }}>
                              <img src={receipt.receiptData} alt={`${stage} payment receipt`}
                                style={{ width: '100%', maxHeight: 220, objectFit: 'contain', borderRadius: 8, border: '1px solid var(--border)', marginBottom: 10 }} />
                              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
                                {receipt.detectedRaw && <span>🔍 Detected: <strong>{receipt.detectedRaw}</strong></span>}
                                {receipt.confidence  && <span>Confidence: {receipt.confidence}</span>}
                                {receipt.uploadedAt  && <span>📅 {new Date(receipt.uploadedAt).toLocaleString()}</span>}
                              </div>
                              {isPending && !isConfirmed && (
                                <button onClick={() => confirmManualPayment(stage)}
                                  ...
                                  {confirmingPayment ? '⏳ Confirming...' : `✅ Confirm ${stage === 'advance' ? 'Advance' : 'Final'} Payment`}
                                </button>
                              )}
                              {isConfirmed && (
                              ...
                              <a href={receipt.receiptData} download={`receipt_${stage}_${selectedRequest.id?.slice(0,6)}`}
                                style={{ display: 'block', textAlign: 'center', marginTop: 8, fontSize: 12, color: 'var(--teal)', fontWeight: 600 }}>
                                ⬇️ Download Receipt
                              </a>
                            </div>
                          </div>
