...
export default function AdminPage() {
  ...
  const updateRequest = async (id, data) => {
    ...
    try {
      ...
      // ── Payment split on completion ─────────────────────
      if (data.status === 'completed' && (data.agreedPrice || req?.agreedPrice)) {
        const total     = Number(data.agreedPrice || req.agreedPrice || 0);
        const writerPay = Math.round(total * 0.85);
        const adminPay  = Math.round(total * 0.10);
        const maintPay  = Math.round(total * 0.05);
        await addDoc(collection(db, 'paymentSplits'), {
          orderId:      id,
          clientId:     req?.userId,
          writerId:     req?.assignedWriterId || null,
          totalAmount:  total,
          writerAmount: writerPay,
          adminAmount:  adminPay,
          maintenanceAmount: maintPay,
