...
export default function AdminPage() {
  ...
  const subscribeOrderChat = (requestId) => {
    ...
    const q = query(
      collection(db, 'projectChats', requestId, 'messages'),
      orderBy('createdAt', 'asc')
    );
    const unsub = onSnapshot(q, snap => {
      ...
      snap.docs.forEach(d => {
        ...
        if ((msg.senderRole === 'client' || msg.senderRole === 'writer') && !msg.readByAdmin) {
          updateDoc(doc(db, 'projectChats', requestId, 'messages', d.id), { readByAdmin: true }).catch(() => {});
        }
      });
  ...
  const sendOrderChatMsg = async () => {
    ...
    try {
      await addDoc(collection(db, 'projectChats', selectedRequest.id, 'messages'), {
        text:         payload,
        senderId:     'admin',
        senderName:   'Admin',
        senderRole:   'admin',
            ...
            {selectedRequest && (
              <div className="req-detail-panel" style={{ ...s.card, position: 'sticky', top: 80, maxHeight: 'calc(100vh - 120px)', overflowY: 'auto' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
                  Back-and-forth with client &amp; writer about this order.
                </div>
