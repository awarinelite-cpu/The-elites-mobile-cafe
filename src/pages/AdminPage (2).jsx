...
export default function AdminPage() {
  ...
  const parsePastedTopics = (raw) => {
    ...
    for (let line of lines) {
      let title = line.trim();
      if (!title) continue;
      // Strip numbered list prefixes: 1. 1) 1: (1) and roman numerals i. ii. iii.
      title = title
        .replace(/^\(?\d+[).:\-]\s*/, '')       // 1. 1) 1: (1)
        .replace(/^[ivxlcdmIVXLCDM]+\.\s+/, '')  // i. ii. iii. iv.
        .replace(/^[•\-\*–—►▶▸>\u25CF\u25AA\u2192]\s*/, '') // bullets
        .replace(/^\d+\s+/, '')                   // bare number space
        .trim();
      if (!title || title.length < 5) continue;
      if (/^\d+$/.test(title)) continue; // skip bare numbers
      parsed.push({ title, category: 'Nursing Science', pages: '', price: '', description: '', badge: '' });
    }
    if (parsed.length === 0) {
      showToast('⚠️ No topics found. Make sure each topic is on its own line.');
      return;
    }
    setBulkParsed(parsed);
    // Stay in paste panel — preview table renders below the textarea
    showToast(`✅ ${parsed.length} topics parsed — review below, then upload.`);
  };
                ...
                {bulkParsed.length === 0 && (
                  <>
                    <div style={{ background: 'rgba(124,58,237,0.06)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.8 }}>
                      Paste any list of research topics — numbered, bulleted, or plain. The system strips numbering and bullets automatically. <strong style={{ color: 'var(--text-primary)' }}>One topic per line.</strong>
                      <br />
                      <span style={{ fontFamily: 'monospace', fontSize: 12, color: '#7C3AED' }}>
                        Works with: &nbsp; 1. Topic title &nbsp;|&nbsp; • Topic title &nbsp;|&nbsp; - Topic title &nbsp;|&nbsp; i. Topic title &nbsp;|&nbsp; Topic title
                      </span>
                    </div>
                    ...
                    <textarea
                      ...
                      placeholder={`Paste your topics here, e.g:\n\n1. Effect of malaria on pregnancy outcomes in rural Nigeria\n2. Knowledge and practice of infection control among nurses\n3. Assessment of antenatal care utilization among pregnant women\n• Impact of community health education on child mortality\n- Prevalence of hypertension among adults in Lagos\ni. Role of midwives in maternal health outcomes`}
                      rows={12}
                      style={{ ...s.input, resize: 'vertical', fontFamily: 'monospace', fontSize: 13, lineHeight: 1.7, marginBottom: 12 }}
                    />
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                      <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                        Review and edit each topic, then upload when ready.
                      </div>
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        <button style={{ ...s.btn('#EF4444'), padding: '7px 14px', fontSize: 12 }} onClick={() => setBulkParsed([])}>
                          Clear All
                        </button>
                      </div>
                    </div>
