        ...
        try {
          ...
          if (order.assignedWriterId) {
            await addDoc(collection(db, 'notifications'), {
              ...
              title: '💰 Client Paid Advance',
              body: `Client paid 50% advance for "${order.serviceTitle || order.topicTitle || 'order'}". Work can begin now.`,
              type: 'payment', read: false, createdAt: ts(),
            });
          }
          setPaying(null);
    ...
    initializePayment({
      ...
      onSuccess: async (response) => {
        try {
          ...
          if (writerId) {
            await addDoc(collection(db, 'notifications'), {
              userId: writerId, title: '💰 Final Payment Received',
              body: `Client completed full payment for "${order.serviceTitle || order.topicTitle || 'order'}".`,
              type: 'payment', read: false, createdAt: ts(),
            });
          }
          setPaying(null);
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', paddingTop: 64, position: 'relative', overflow: 'hidden' }}>
      ...
      <main className="dash-main" style={{ flex: 1, padding: 'clamp(20px,3vw,36px)', overflowY: 'auto', minWidth: 0 }}>
        ...
        {tab === 'overview'  && <Overview stats={stats} orders={orders} loading={loading} s={s} paying={paying} onSelectOrder={setSelectedOrder} />}
        {tab === 'orders'    && <OrdersTab orders={orders} loading={loading} s={s} paying={paying} user={user} profile={profile} onSelectOrder={setSelectedOrder} />}
...
// ── Order Detail View ─────────────────────────────────────────
function OrderDetailView({ order: initialOrder, user, profile, onBack, onPayAdvance, onPayFinal, paying }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', paddingTop: 64, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 'clamp(16px,3vw,28px)', display: 'flex', flexDirection: 'column', gap: 16, maxWidth: 720, width: '100%', margin: '0 auto' }}>
        ...
        {/* Accepted — pay 50% advance */}
        {order.status === 'accepted' && (order.agreedPrice || order.totalAmount) && !order.advancePaid && (
          <div>
            ...
            <button onClick={() => onPayAdvance(order)} disabled={paying === order.id}
              ...
              {paying === order.id ? '⏳ Opening payment...' : `💳 Pay 50% Advance — ₦${Math.round(Number(order.agreedPrice || order.totalAmount) / 2).toLocaleString()}`}
            </button>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', textAlign: 'center', marginTop: 8 }}>Work begins after advance payment. Final 50% due on completion.</div>

            {/* ── Manual Payment Alternative ── */}
            <div style={{ marginTop: 14 }}>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', textAlign: 'center', marginBottom: 8 }}>— or pay manually via bank transfer —</div>
              <ManualPaymentUpload order={order} paymentStage="advance" onDone={() => {}} />
            </div>
          </div>
        )}

        {/* In progress — advance paid, show progress */}
        {order.status === 'in_progress' && order.advancePaid && !order.finalPaid && (order.agreedPrice || order.totalAmount) && (
        ...
        {/* Completed — pay final 50% */}
        {order.status === 'completed' && order.advancePaid && !order.finalPaid && (order.agreedPrice || order.totalAmount) && (
          <div>
            <div style={{ background: 'rgba(22,163,74,0.07)', border: '1px solid rgba(22,163,74,0.3)', borderRadius: 10, padding: '16px', marginBottom: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: '#16A34A', marginBottom: 6 }}>🎉 Work Completed!</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <button onClick={() => onPayFinal(order)} disabled={paying === order.id}
              style={{ width: '100%', background: paying === order.id ? 'rgba(22,163,74,0.4)' : '#16A34A', color: '#fff', border: 'none', borderRadius: 10, padding: '14px', cursor: paying === order.id ? 'not-allowed' : 'pointer', fontSize: 15, fontWeight: 700, fontFamily: 'var(--font-body)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              {paying === order.id ? '⏳ Opening payment...' : `💳 Pay Final 50% — ₦${Math.round(Number(order.agreedPrice || order.totalAmount) / 2).toLocaleString()}`}
            </button>
        ...
        {/* Fully paid — download delivery */}
        {order.finalPaid && order.draftFileData && (
          <div style={{ background: 'rgba(22,163,74,0.08)', border: '2px solid rgba(22,163,74,0.4)', borderRadius: 12, padding: '20px' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#16A34A', marginBottom: 6 }}>🎉 Fully Paid — Your Work is Ready!</div>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 14 }}>Thank you for your payment. Download your completed work below.</div>
            <a
        ...
        {order.finalPaid && !order.draftFileData && (
          <div style={{ background: 'rgba(22,163,74,0.07)', border: '1px solid rgba(22,163,74,0.25)', borderRadius: 10, padding: '14px 16px' }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: '#16A34A', marginBottom: 4 }}>✅ Payment Complete</div>
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>Your full payment has been received. The admin will upload your completed work shortly.</div>
          </div>
        )}
          ...
          {orders.slice(0, 5).map(o => (
            <div key={o.id} onClick={() => onSelectOrder(o)}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                ...
                {o.status === 'completed' && o.advancePaid && !o.finalPaid && (
                  <span style={{ fontSize: 11, color: '#fff', background: '#16A34A', borderRadius: 6, padding: '3px 9px', fontWeight: 700 }}>💳 Pay balance</span>
                )}
                {o.finalPaid && o.draftFileData && (
                  <span style={{ fontSize: 11, color: '#fff', background: '#16A34A', borderRadius: 6, padding: '3px 9px', fontWeight: 700 }}>⬇️ Download ready</span>
                )}
            ...
            <div key={o.id} onClick={() => onSelectOrder(o)}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 8 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 6 }}>
                  ...
                  {o.status === 'completed' && o.advancePaid && !o.finalPaid && (
                    <span style={{ fontSize: 11, color: '#fff', background: '#16A34A', borderRadius: 6, padding: '3px 9px', fontWeight: 700 }}>💳 Pay balance to download</span>
                  )}
                  {o.finalPaid && o.draftFileData && (
                    <span style={{ fontSize: 11, color: '#fff', background: '#16A34A', borderRadius: 6, padding: '3px 9px', fontWeight: 700 }}>⬇️ Ready to download</span>
                  )}
                </div>
              </div>
