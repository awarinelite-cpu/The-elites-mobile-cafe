...
function ServicesTab({ user, profile, onOrderCreated, s }) {
  const [selected, setSelected] = useState(null); // selected service key
  ...
  const submit = async () => {
    if (!form.details?.trim()) { alert('Please describe what you need.'); return; }
  ...
  if (submitted) return (
    <div style={{ textAlign: 'center', padding: '60px 24px' }}>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>Admin will review and respond shortly. Redirecting to My Orders…</p>
    </div>
  );
  ...
  if (selected && service) return (
    <div>
      ...
      <div style={{ background: 'var(--teal-glow)', border: '1px solid rgba(13,148,136,0.3)', borderRadius: 12, padding: '16px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 14 }}>
        <span style={{ fontSize: 32 }}>{service.icon}</span>
        <div>
          <div style={{ fontWeight: 700, fontSize: 17, color: 'var(--text-primary)' }}>{service.title}</div>
          <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 3 }}>{service.desc}</div>
        </div>
      </div>

      <div style={{ background: 'rgba(201,168,76,0.07)', border: '1px solid rgba(201,168,76,0.2)', borderRadius: 10, padding: '12px 16px', marginBottom: 24, fontSize: 13, color: 'var(--gold)' }}>
        📋 After submitting, admin will review your request and send a price quote. No payment until you agree.
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {/* Details */}
        <div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Project Details *</label>
          <textarea name="details" rows={5} value={form.details || ''} onChange={handle}
            placeholder={`Describe exactly what you need for ${service.title}. The more detail, the better we can help you.`}
        ...
        {/* Phone */}
        <div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Phone Number</label>
          <input name="phone" value={form.phone || profile?.phone || ''} onChange={handle}
            placeholder="e.g. 08012345678"
            style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-tertiary)', border: '1.5px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontFamily: 'var(--font-body)', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
        </div>

        {/* Pages — for research/writing services */}
        {['research_projects','academic_assignments','proofreading','survey_design'].includes(service.key) && (
          <div>
            <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Number of Pages</label>
            <select name="pages" value={form.pages || ''} onChange={handle}
              style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-tertiary)', border: '1.5px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontFamily: 'var(--font-body)', fontSize: 14, outline: 'none', boxSizing: 'border-box' }}>
              <option value="">Select range…</option>
              {['5–10','10–15','15–20','20–30','30–50','50+','Custom'].map(p => <option key={p} value={p}>{p} pages</option>)}
            </select>
          </div>
        )}
        ...
        {/* Deadline */}
        <div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Preferred Deadline</label>
          <input name="deadline" type="date" value={form.deadline || ''} onChange={handle}
            style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-tertiary)', border: '1.5px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontFamily: 'var(--font-body)', fontSize: 14, outline: 'none', boxSizing: 'border-box', colorScheme: 'dark' }} />
        </div>

        {/* Additional notes */}
        <div>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 }}>Additional Notes</label>
          <textarea name="notes" rows={2} value={form.notes || ''} onChange={handle}
            placeholder="Any other details, preferences or requirements…"
            style={{ width: '100%', padding: '10px 12px', background: 'var(--bg-tertiary)', border: '1.5px solid var(--border)', borderRadius: 8, color: 'var(--text-primary)', fontFamily: 'var(--font-body)', fontSize: 13, outline: 'none', resize: 'none', boxSizing: 'border-box' }} />
        </div>

        <button onClick={submit} disabled={submitting || !form.details?.trim()}
          style={{ background: submitting ? 'rgba(13,148,136,0.5)' : 'var(--teal)', color: '#fff', border: 'none', borderRadius: 10, padding: '14px', cursor: submitting ? 'not-allowed' : 'pointer', fontSize: 15, fontWeight: 700, fontFamily: 'var(--font-body)', opacity: form.details?.trim() ? 1 : 0.6 }}>
          {submitting ? '⏳ Submitting…' : `📤 Submit ${service.title} Request`}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 24 }}>Choose a service to submit a request. Admin will review and send you a quote.</p>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {SERVICES.map(sv => (
