  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', display: 'flex', paddingTop: 64 }}>
      ...
      <main style={{ flex: 1, padding: 'clamp(20px,3vw,36px)', overflowY: 'auto', minWidth: 0 }}>
        {tab === 'overview'  && <Overview  stats={stats} orders={orders} loading={loading} s={s} />}
        {tab === 'orders'    && <OrdersTab orders={orders} loading={loading} s={s} />}
...
// ── Overview ─────────────────────────────────────────────────
function Overview({ stats, orders, loading, s }) {
  return (
    <div>
...
// ── Orders ────────────────────────────────────────────────────
function OrdersTab({ orders, loading, s }) {
...
function MessagesTab({ user, profile }) {
  ...
  const sendAudio = async () => {
    ...
    setSending(true);
    try {
      const audioRef = ref(storage, `voiceNotes/client_${user.uid}_${Date.now()}.webm`);
            ...
            return (
              <div key={msg.id} style={{ display: 'flex', justifyContent: isMe ? 'flex-end' : 'flex-start' }}>
                <div style={{
                  ...
                  {msg.type === 'audio'
                    ? <audio controls src={msg.audioURL} style={{ maxWidth: 220, height: 36, display: 'block' }} />
...
function PaymentsTab({ orders, stats }) {
  const paid = orders.filter(o => o.agreedPrice || o.totalAmount);
  return (
    <div>
          ...
          {paid.map(o => (
            <div key={o.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-card)', borderRadius: 12, padding: 20, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
              <div>
                ...
                <span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700, textTransform: 'uppercase', background: STATUS_COLORS[o.status]?.bg || '#eee', color: STATUS_COLORS[o.status]?.color || '#333' }}>{o.status?.replace('_',' ')}</span>
              </div>
