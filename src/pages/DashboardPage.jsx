    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
        ...
        <button onClick={() => setActiveCategory('All')}
          style={{ alignSelf: 'flex-start', padding: '7px 22px', borderRadius: 20, border: `2px solid ${activeCategory === 'All' ? 'var(--teal)' : 'var(--border)'}`, background: activeCategory === 'All' ? 'var(--teal)' : 'var(--bg-card)', color: activeCategory === 'All' ? '#fff' : 'var(--text-muted)', cursor: 'pointer', fontWeight: 700, fontSize: 13, fontFamily: 'var(--font-body)', transition: 'all 0.2s' }}>
          🗂️ All Departments
        </button>

        {CATEGORY_GROUPS.map(grp => {
          const isGroupActive = grp.cats.includes(activeCategory);
          return (
            <div key={grp.group} style={{ background: 'var(--bg-card)', border: `1.5px solid ${isGroupActive ? grp.color + '60' : 'var(--border-card)'}`, borderRadius: 12, overflow: 'hidden', transition: 'border-color 0.2s' }}>
              {/* Group header */}
              <div style={{ padding: '11px 14px', background: isGroupActive ? grp.bg : 'transparent', display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={{ fontWeight: 700, fontSize: 13, color: isGroupActive ? grp.color : 'var(--text-primary)' }}>{grp.group}</span>
                {isGroupActive && (
                  <span style={{ fontSize: 10, fontWeight: 700, color: grp.color, background: grp.bg, border: `1px solid ${grp.color}40`, borderRadius: 12, padding: '2px 8px' }}>
                    {activeCategory}
                  </span>
                )}
              </div>
              {/* Department pills */}
              <div style={{ padding: '10px 12px 12px', display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                {grp.cats.map(cat => {
                  const isActive = activeCategory === cat;
                  return (
                    <button key={cat} onClick={() => setActiveCategory(isActive ? 'All' : cat)}
                      style={{ padding: '5px 14px', borderRadius: 20, border: `1.5px solid ${isActive ? grp.color : 'var(--border)'}`, background: isActive ? grp.color : 'var(--bg-secondary)', color: isActive ? '#fff' : 'var(--text-muted)', cursor: 'pointer', fontWeight: isActive ? 700 : 500, fontSize: 12, fontFamily: 'var(--font-body)', whiteSpace: 'nowrap', transition: 'all 0.2s' }}>
                      {cat}
                    </button>
                  );
                })}
        ...
        {/* Other pill */}
        <button onClick={() => setActiveCategory(activeCategory === 'Other' ? 'All' : 'Other')}
          style={{ alignSelf: 'flex-start', padding: '5px 14px', borderRadius: 20, border: `1.5px solid ${activeCategory === 'Other' ? 'var(--text-muted)' : 'var(--border)'}`, background: activeCategory === 'Other' ? 'var(--bg-secondary)' : 'transparent', color: 'var(--text-muted)', cursor: 'pointer', fontWeight: 500, fontSize: 12, fontFamily: 'var(--font-body)', transition: 'all 0.2s' }}>
          Other
        </button>
      </div>
