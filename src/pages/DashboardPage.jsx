...
import { doc, updateDoc, addDoc, collection, serverTimestamp as ts, writeBatch, getDocs } from 'firebase/firestore';
...
export default function DashboardPage() {
  ...
  const [unreadOrderIds, setUnreadOrderIds] = useState(new Set()); // orders with unread admin messages
  const [unreadMsgCount, setUnreadMsgCount] = useState(0);         // unread general messages from admin
  const seenStatusRef = useRef({});  // tracks last seen status per order to detect changes
  ...
  // ── Track orders with status changes the client hasn't seen yet ──
  useEffect(() => {
    ...
    orders.forEach(o => {
      ...
      if (prev && prev !== o.status) {
        // Status changed since last seen — mark as unread
        newUnread.add(o.id);
      }
      // If not seen before, initialise without marking unread
      if (!prev) seenStatusRef.current[o.id] = o.status;
    });
    setUnreadOrderIds(newUnread);
  }, [orders]);

  // ── Track unread admin messages (general chat) ──
  useEffect(() => {
    if (!user) return;
    const q = query(
  ...
  const handleTabChange = async (id) => {
    ...
    // Clear unread badges when user visits the relevant tab
    if (id === 'orders') {
      // Mark all current orders' statuses as seen
      orders.forEach(o => { seenStatusRef.current[o.id] = o.status; });
      setUnreadOrderIds(new Set());
    }
    if (id === 'messages') {
      setUnreadMsgCount(0);
      // Mark all unread admin messages as read in Firestore
      if (user) {
        try {
          const q = query(
            collection(db, 'adminMessages', user.uid, 'messages'),
          ...
          snap.docs.forEach(d => {
            if (!d.data().readByClient) {
              batch.update(d.ref, { readByClient: true });
            }
          });
          await batch.commit();
