...
export default function AIResearchWriterPage() {
  ...
  const handleGuideFile = (e) => {
    ...
    sessionStorage.setItem('aiw_guidename', file.name);
    const reader = new FileReader();
    reader.onloadend = () => {
      const text = reader.result || '';
      setGuideDoc(text);
      sessionStorage.setItem('aiw_guide', text);
    };
...
You MUST follow the structure, headings, subheadings, writing style, paragraph length, table format, and all formatting conventions shown in this guide document EXACTLY. This guide overrides all other formatting instructions. Do NOT skip any section shown in the guide. Do NOT add sections not in the guide. Do NOT produce half-complete work — every section must be fully written to the same depth and length as the guide.
...
Include sub-sections: [Practice 1], [Practice 2], [Practice 3], [Practice 4], [Practice 5], [Practice 6]
...
Narrative paragraph describing demographics. End with: "This demographic profile provides a robust foundation for analyzing [variable 1 and variable 2] among [population] in [facility]."
...
- Para 1: State 2–3 findings with percentages from Chapter 4.
- Para 2: Compare with global/high-income country study.
- Para 3: Compare with African study.
- Para 4: Compare with Nigerian study. Add analytical remark.
...
1–2 paragraphs: sample confined to one facility, self-report bias, cross-sectional design. Cite 1 source.

5.4 Summary [bold, NO section number]
1 paragraph: state dominant demographics, key findings (with %), hypothesis outcomes (χ² values and p-values).

5.5 Conclusion [bold, NO section number]
1–2 paragraphs with specific percentages. Acknowledge gaps. Forward-looking statement. Cite 1–2 sources.

5.6 Recommendations [bold, NO section number]
6–8 items. Each: [Bold title]: [1–2 sentence explanation with cited study.]

5.7 Suggestions for Further Studies [bold, NO section number]
6 items. Each: [Bold title]: [1–2 sentence description of study, where, outcomes to measure.]
...
Alphabetical by first author surname. Minimum 25–30 unique entries. 2021–2025 only except foundational theories.
...
2.1 Conceptual Review — pathophysiology, epidemiology (Nigerian/African data), aetiology and risk factors
2.2 Clinical Manifestations and Assessment — signs, symptoms, diagnostic criteria
2.3 Theoretical Framework — 2.3.1: 2–3 nursing theories (proponent and year); 2.3.2: Application to study
2.4 Empirical Review — 6–8 real studies (2021–2025); Global → Africa → Nigeria
...
Your writing philosophy: original thought expressed clearly, evidence used to support YOUR analysis (not replace it), every sentence crafted fresh. A plagiarism checker must return 0–5% similarity.
...
- Use numbered subheadings EXACTLY as listed (1.1, 1.2, 2.1.1, etc.)
- Never start with preamble — begin directly with CHAPTER [NUMBER] in ALL CAPS
- Do NOT produce half-complete work — every section fully written

HUMAN WRITING STYLE:
- Every sentence freshly constructed — no recycled templates
- Vary sentence openings — not more than two consecutive sentences starting the same way
- Add analytical commentary after every citation — meaning for Nigerian context
- Natural connectives: "This pattern suggests...", "What emerges from these findings is..."
- Do NOT use: "It is important to note", "Furthermore it should be noted", "In conclusion it can be seen"
...
${ch.subtitle.toUpperCase()}`;
  ...
  const copyAll = () => {
    navigator.clipboard.writeText(activeChapters.map(c=>chapters[c.id]||'').filter(Boolean).join('\n\n\n'));
    toast('📋 Full document copied!','#1E3A8A');
  };

  // ── Download .docx ───────────────────────────────────────────
  const downloadDocx = async () => {
    if (!activeChapters.some(c=>chapters[c.id])) { toast('⚠️ No chapters generated yet','#D97706'); return; }
    setDownloadingDocx(true);
    try {
      if (!window.JSZip) {
        await new Promise((resolve,reject)=>{ const s=document.createElement('script'); s.src='https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js'; s.onload=resolve; s.onerror=()=>reject(new Error('Could not load JSZip.')); document.head.appendChild(s); });
      }
      const esc=(t)=>String(t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
      const mkRun=(text,bold=false)=>`<w:r><w:rPr><w:rFonts w:ascii="Times New Roman" w:hAnsi="Times New Roman" w:cs="Times New Roman"/><w:sz w:val="24"/><w:szCs w:val="24"/>${bold?'<w:b/><w:bCs/>':''}</w:rPr><w:t xml:space="preserve">${esc(text)}</w:t></w:r>`;
      ...
      const monoLine=(t)=>`<w:p><w:pPr><w:spacing w:line="240" w:lineRule="auto" w:before="0" w:after="0"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="Courier New" w:hAnsi="Courier New"/><w:sz w:val="18"/><w:szCs w:val="18"/></w:rPr><w:t xml:space="preserve">${esc(t)}</w:t></w:r></w:p>`;
      const newPage=()=>mkP(mkRun(''),true,false,false,true);
      const boldColon=(text)=>{ const ci=text.indexOf(':'); if(ci<0)return justN(text); return mkP(mkRun(text.slice(0,ci+1),true)+mkRun(text.slice(ci+1),false),false); };
      ...
      const classify=(line)=>{
        const t=line.trim(); if(!t)return 'blank';
        ...
        if(/^(Broad Objective|Specific Objectives|Decision Rule|Inference:|Source:|REFERENCES|Research (Question|Hypothesis)\s+\d+|Hypothesis (One|Two)|Summary|Conclusion|Recommendations|Suggestions for Further Studies|Socio-Demographic|Findings on|Implications|Limitations)/i.test(t))return 'bold-label';
      ...
      const textToXML=(rawText)=>{
        ...
        const flushTable=()=>{ if(!tableBuf.length)return; tableBuf.forEach(r=>{out+=monoLine(r);}); out+=blank(); tableBuf=[]; };
        for(const line of lines){
          const type=classify(line),t=line.trim();
      ...
      for(const ch of activeChapters){if(!chapters[ch.id])continue;bodyXML+=newPage();bodyXML+=textToXML(chapters[ch.id]);}
      ...
      const documentXML=`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><w:body>${bodyXML}${sectProps}</w:body></w:document>`;
      ...
      zip.file('[Content_Types].xml',contentTypes); zip.file('_rels/.rels',relsXML); zip.file('word/_rels/document.xml.rels',wordRelsXML);
      zip.file('word/document.xml',documentXML); zip.file('word/styles.xml',stylesXML); zip.file('word/settings.xml',settingsXML); zip.file('word/footer1.xml',footerXML);
    ...
    } catch(err){ console.error('DOCX error:',err); toast(`❌ Download failed: ${err.message}`,'#DC2626'); }
  ...
  const downloadPdf = async () => {
    ...
    try {
      if (!window.jspdf) {
        await new Promise((resolve,reject)=>{ const s=document.createElement('script'); s.src='https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js'; s.onload=resolve; s.onerror=()=>reject(new Error('Could not load jsPDF.')); document.head.appendChild(s); });
      }
      const {jsPDF}=window.jspdf;
      const doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
      const pageW=doc.internal.pageSize.getWidth(),pageH=doc.internal.pageSize.getHeight();
      const marginL=25,marginR=20,marginT=25,marginB=20,usableW=pageW-marginL-marginR;
      let y=marginT,pageNum=1;
      const addPageNumber=()=>{ doc.setFontSize(10);doc.setFont('times','normal');doc.text(String(pageNum),pageW/2,pageH-10,{align:'center'}); };
      const checkNewPage=(needed=10)=>{ if(y+needed>pageH-marginB){addPageNumber();doc.addPage();pageNum++;y=marginT;} };
      const writeLine=(text,bold=false,center=false,fontSize=12)=>{ doc.setFontSize(fontSize);doc.setFont('times',bold?'bold':'normal'); const x=center?pageW/2:marginL;const align=center?'center':'left'; doc.splitTextToSize(text,usableW).forEach(line=>{checkNewPage(8);doc.text(line,x,y,{align});y+=8;}); };
      const writePara=(text)=>{ doc.setFontSize(12);doc.setFont('times','normal'); doc.splitTextToSize(text,usableW).forEach(line=>{checkNewPage(8);doc.text(line,marginL,y);y+=8;}); y+=4; };
      // Title page
      y=pageH/2-40; doc.setFontSize(14);doc.setFont('times','bold');
      doc.splitTextToSize((topic||'RESEARCH PROJECT').toUpperCase(),usableW).forEach(line=>{doc.text(line,pageW/2,y,{align:'center'});y+=10;});
      y+=6;doc.setFontSize(12);doc.text('BY',pageW/2,y,{align:'center'});y+=10;
      ...
      doc.text(`${new Date().toLocaleString('default',{month:'long'}).toUpperCase()}, ${new Date().getFullYear()}`,pageW/2,y,{align:'center'});
      // Chapters
      for(const ch of activeChapters){
        if(!chapters[ch.id])continue;
        addPageNumber();doc.addPage();pageNum++;y=marginT;
        for(const line of chapters[ch.id].split('\n')){
          const t=line.trim(); if(!t){y+=4;continue;}
          if(/^CHAPTER\s+(ONE|TWO|THREE|FOUR|FIVE)$/i.test(t)){checkNewPage(14);writeLine(t.toUpperCase(),true,true,14);y+=2;}
          else if(t===t.toUpperCase()&&/^[A-Z][A-Z\s,&/\-]+$/.test(t)&&t.length>=4&&t.length<90&&!/^\d/.test(t)){writeLine(t.toUpperCase(),true,true,13);y+=2;}
          else if(/^\d+\.\d+(\.\d+)?\s+\S/.test(t)){y+=2;checkNewPage(12);writeLine(t,true,false,12);y+=2;}
          else if(/^H[₀oO0][₁₂₃123][:.\s]/.test(t)||/^H[₁1][₁₂₃123][:.\s]/.test(t)){writeLine(t,true,false,12);}
          else if(/^(REFERENCES|Summary|Conclusion|Recommendations|Suggestions|Socio-Demographic|Findings on|Implications|Limitations|Hypothesis (One|Two)|Decision Rule)/i.test(t)){y+=2;checkNewPage(12);writeLine(t,true,false,12);y+=2;}
          else if(t.startsWith('|')||(t.includes('|')&&t.indexOf('|')<5)){doc.setFontSize(9);doc.setFont('courier','normal');doc.splitTextToSize(t,usableW).forEach(l=>{checkNewPage(6);doc.text(l,marginL,y);y+=6;});}
          else if(/^[A-Z][A-Za-z\s]+:/.test(t)&&t.length<140&&t.split(':')[0].split(' ').length<=10){
            const ci=t.indexOf(':');checkNewPage(8);doc.setFontSize(12);doc.setFont('times','bold');
            const bW=doc.getTextWidth(t.slice(0,ci+1));doc.text(t.slice(0,ci+1),marginL,y);
            doc.setFont('times','normal');const rest=doc.splitTextToSize(t.slice(ci+1),usableW-bW-2);
            doc.text(rest[0]||'',marginL+bW+1,y);y+=8;
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg-primary,#0f172a)',paddingTop:64,paddingBottom:60}}>
      ...
      {/* Header */}
      <div style={{background:mode==='clientcare'?'linear-gradient(135deg,#7C3AED,#0D9488)':'linear-gradient(135deg,#1E3A8A,#0D9488)',padding:'24px 20px'}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            ...
            <div>
              ...
              <p style={{color:'rgba(255,255,255,0.75)',fontSize:12,margin:'3px 0 0'}}>
                {totalChapters} chapters · {citationStyle} 7th ed. · .docx & .pdf download
              </p>
            </div>
          </div>
          <div style={{background:'rgba(0,0,0,0.3)',borderRadius:12,padding:4,display:'flex',gap:3}}>
            {[['research','🔬 Research','#1E3A8A'],['clientcare','🏥 Client Care','#7C3AED']].map(([m,label,col])=>(
              <button key={m} onClick={()=>{setMode(m);setChapters({});setActiveChapter(null);setError('');}}
                style={{padding:'7px 13px',borderRadius:9,border:'none',cursor:'pointer',fontWeight:700,fontSize:12,fontFamily:'var(--font-body,inherit)',whiteSpace:'nowrap',background:mode===m?'#fff':'transparent',color:mode===m?col:'rgba(255,255,255,0.75)'}}>
                {label}
              </button>
            ))}
          </div>
      ...
      <div style={{maxWidth:900,margin:'0 auto',padding:'20px 16px',display:'flex',flexDirection:'column',gap:18}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:15}}>
            <div>
              ...
              <textarea value={topic} onChange={e=>setTopic(e.target.value)} rows={3}
                placeholder={mode==='clientcare'?'e.g. Nursing Care of a 48-Year-Old Female Patient with Type 2 Diabetes Mellitus':'e.g. Assessment of Knowledge and Practice of Infection Prevention Among Nurses in Lagos State University Teaching Hospital'}
            ...
            <div style={{background:'var(--bg-tertiary,#0f172a)',border:'1px solid var(--border,#2d3748)',borderRadius:10,padding:'14px 16px'}}>
              ...
              <div style={{fontSize:11,color:'var(--text-muted,#718096)',marginTop:8}}>💡 e.g. <strong style={{color:'var(--text-primary,#e2e8f0)'}}>10-15</strong> or <strong style={{color:'var(--text-primary,#e2e8f0)'}}>20</strong></div>
            </div>
          </div>
        </div>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10}}>
              ...
              <div style={{textAlign:'left'}}>
                ...
                <div style={{fontSize:12,color:'var(--text-muted,#718096)',marginTop:2}}>
                  {guideDoc.trim()?`✅ Loaded${guideFileName?` — ${guideFileName}`:''} · ${guideDoc.trim().split(/\s+/).length.toLocaleString()} words`:'Upload or paste a sample study — AI will strictly follow its format'}
                </div>
              </div>
            </div>
            <span style={{color:accentColor,fontSize:18,transform:showGuide?'rotate(180deg)':'none'}}>▾</span>
          </button>
          {showGuide&&(
            <div style={{padding:'0 22px 22px',borderTop:'1px solid var(--border,#2d3748)'}}>
              <div style={{background:'rgba(37,99,235,0.07)',border:'1px solid rgba(37,99,235,0.2)',borderRadius:8,padding:'10px 14px',margin:'14px 0',fontSize:13,color:'#60A5FA'}}>
                ℹ️ The AI treats this as <strong>mandatory format authority</strong> — strictly following structure, headings, paragraph depth, and style. Max 5MB.
              </div>
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
                ...
                <span style={{fontSize:12,color:'var(--text-muted,#718096)'}}>.txt recommended · max 5MB</span>
                {guideDoc.trim()&&(
                  <button onClick={()=>{setGuideDoc('');setGuideFileName('');sessionStorage.removeItem('aiw_guide');sessionStorage.removeItem('aiw_guidename');}}
              ...
              <textarea value={guideDoc} onChange={e=>{setGuideDoc(e.target.value);setGuideFileName('');sessionStorage.setItem('aiw_guide',e.target.value);}} rows={8}
                placeholder="Paste an existing research project here. The AI will strictly follow its format."
        ...
        <div style={{background:'rgba(22,163,74,0.07)',border:'1px solid rgba(22,163,74,0.25)',borderRadius:10,padding:'10px 16px',display:'flex',gap:10,alignItems:'flex-start'}}>
          ...
          <div style={{fontSize:12,color:'rgba(22,163,74,0.9)',lineHeight:1.6}}>
            <strong>Citation Policy:</strong> Every paragraph contains in-text citations. All references consolidated at end of Chapter 5 only. No fabricated sources. 2021–2025 only.
          </div>
        </div>
        <div style={{background:'rgba(37,99,235,0.07)',border:'1px solid rgba(37,99,235,0.25)',borderRadius:10,padding:'10px 16px',display:'flex',gap:10,alignItems:'flex-start'}}>
          ...
          <div style={{fontSize:12,color:'rgba(147,197,253,0.9)',lineHeight:1.6}}>
            <strong>Download:</strong> <strong>Times New Roman 12pt · Double spacing · Justified · A4 · 1.5" left margin</strong>. Available as <strong>Word (.docx)</strong> and <strong>PDF</strong>.
          </div>
        </div>
        ...
        <button onClick={generateAll} disabled={!canGenerate||generatingAll||loading!==null}
          style={{width:'100%',padding:'15px',borderRadius:12,border:'none',fontSize:15,fontWeight:700,fontFamily:'var(--font-body,inherit)',cursor:canGenerate&&!generatingAll&&!loading?'pointer':'not-allowed',display:'flex',alignItems:'center',justifyContent:'center',gap:10,
        ...
        <div style={{display:'grid',gridTemplateColumns:`repeat(${totalChapters},1fr)`,gap:8}}>
          {activeChapters.map(ch=>{
            const done=!!chapters[ch.id],isLoading=loading===ch.id;
            return (
          ...
          <div style={{background:'var(--bg-card,#1a2236)',borderRadius:10,padding:'14px 18px',border:'1px solid var(--border-card,#2d3748)'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8,flexWrap:'wrap',gap:8}}>
              ...
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                <button onClick={copyAll} style={{background:'#1E3A8A',color:'#fff',border:'none',borderRadius:8,padding:'7px 14px',cursor:'pointer',fontWeight:700,fontSize:12}}>
                  📋 Copy All
                </button>
        ...
        {!activeChapter&&generatedCount===0&&(
          <div style={{textAlign:'center',padding:'48px 24px',background:'var(--bg-card,#1a2236)',borderRadius:14,border:'1px solid var(--border-card,#2d3748)'}}>
            ...
            <p style={{color:'var(--text-muted,#718096)',fontSize:14,maxWidth:460,margin:'0 auto 24px',lineHeight:1.7}}>
              {mode==='clientcare'
                ?<>Fill in the client condition and objectives, then click <strong style={{color:'#7C3AED'}}>Generate All {totalChapters} Care Study Chapters</strong>.</>
                :<>Fill in your research topic and objectives, then click <strong style={{color:'#0D9488'}}>Generate All {totalChapters} Chapters</strong>. Upload a guide document to enforce strict formatting.</>}
            </p>
            <div style={{display:'flex',gap:10,justifyContent:'center',flexWrap:'wrap'}}>
              {activeChapters.map(ch=>(
                <div key={ch.id} style={{padding:'7px 14px',borderRadius:20,border:`1px solid ${ch.border}`,fontSize:12,fontWeight:600,color:ch.color,background:ch.bg}}>{ch.title}</div>
              ))}
            </div>
          </div>
        )}
