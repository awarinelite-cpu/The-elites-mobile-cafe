...
- References (list 15-20 plausible academic references in ${citationStyle} format, consistent with in-text citations used throughout)`,
    ...
    return `You are an expert Nigerian academic research writer with deep knowledge of nursing, public health, medicine and allied health sciences.

Write ${ch.title}: ${ch.subtitle} for the following research project:
...
TARGET LENGTH: Approximately ${pages} pages total for the full project
...
Start with: ${ch.title.toUpperCase()}\n${ch.subtitle.toUpperCase()}`;
  ...
  const generatedCount = CHAPTERS.filter(c => chapters[c.id]).length;
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary,#0f172a)', paddingTop: 64, paddingBottom: 60 }}>
      ...
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg,#1E3A8A 0%,#0D9488 100%)', padding: '28px 24px', marginBottom: 0 }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 6 }}>
            <div style={{ fontSize: 32 }}>🤖</div>
            <div>
              <h1 style={{ fontFamily: 'var(--font-display,serif)', fontSize: 'clamp(20px,3vw,28px)', fontWeight: 700, color: '#fff', margin: 0 }}>
                AI Research Writer
              </h1>
              <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 13, margin: '3px 0 0' }}>
                Enter your topic & objectives — AI writes all 5 chapters instantly
              </p>
            </div>
          </div>
        </div>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px' }}>
        ...
        <div style={{ background: 'var(--bg-card,#1a2236)', border: '1px solid var(--border-card,#2d3748)', borderRadius: 14, padding: '24px', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            {/* Topic */}
            <div>
              <label style={lbl}>Research Topic *</label>
              <textarea
                value={topic}
                onChange={e => setTopic(e.target.value)}
                placeholder="e.g. Assessment of Knowledge and Practice of Infection Prevention and Control Among Nurses in Lagos State University Teaching Hospital"
                rows={3}
        ...
        <button
          ...
          ) : '🚀 Generate All 5 Chapters At Once'}
        </button>
        ...
        {!activeChapter && generatedCount === 0 && (
          <div style={{ textAlign: 'center', padding: '48px 24px', background: 'var(--bg-card,#1a2236)', borderRadius: 14, border: '1px solid var(--border-card,#2d3748)' }}>
            ...
            <h3 style={{ fontFamily: 'var(--font-display,serif)', fontSize: 20, color: 'var(--text-primary,#e2e8f0)', marginBottom: 8 }}>Ready to Write</h3>
            <p style={{ color: 'var(--text-muted,#718096)', fontSize: 14, maxWidth: 400, margin: '0 auto 24px' }}>
              Fill in your research topic and objectives above, then click <strong style={{ color: '#0D9488' }}>Generate All 5 Chapters</strong> or select individual chapters to generate one at a time.
            </p>
