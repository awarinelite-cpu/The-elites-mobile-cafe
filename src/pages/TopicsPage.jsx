...
import { Search, Filter, ArrowRight, ArrowLeft, SortAsc } from 'lucide-react';
...
export default function TopicsPage() {
  ...
  useEffect(() => {
    const unsub = subscribeToTopics(data => {
      setTopics(data.filter(t => t.available !== false)); // hide unavailable by default
      setLoading(false);
    });
    return unsub;
  }, []);
    ...
    <div style={{ minHeight: '100vh', background: 'var(--dark)', paddingTop: 80 }}>
        ...
        <div style={{ maxWidth: 1200, margin: '0 auto' }}>
          ...
          <div style={{ position: 'relative', maxWidth: 520 }}>
            ...
            <input 
              ...
              placeholder="Search by topic title, keywords, or description..."
              style={{ 
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px clamp(16px,4vw,60px)' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center', marginBottom: 28, paddingBottom: 16, borderBottom: '1px solid var(--border)' }}>
          {/* Category Chips */}
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', overflowX: 'auto', paddingBottom: 4, flex: 1 }}>
            {CATEGORIES.map(cat => (
              <button 
                key={cat} 
              ...
              >
                {cat} {categoryCounts[cat] > 0 && <span style={{ opacity: 0.7 }}>({categoryCounts[cat]})</span>}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <EmptyState 
            ...
            desc={search ? `No results for "${search}" in this category.` : 'No topics available yet in this category.'} 
          />
        ) : (
...
// TopicCard remains mostly the same but with small polish
