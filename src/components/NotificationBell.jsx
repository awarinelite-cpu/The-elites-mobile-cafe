...
export default function NotificationBell() {
  ...
  // Admin gets both their own uid channel AND the shared 'admin' channel
  useEffect(() => {
    if (!user) return;
    const isAdmin = profile?.isAdmin;

    if (isAdmin) {
      // Subscribe to both channels and merge
      const map = {};
      const merge = () => {
      ...
      const unsub1 = subscribeToNotifications(user.uid, (data) => { map['uid'] = data; merge(); });
      const unsub2 = subscribeToNotifications('admin',   (data) => { map['admin'] = data; merge(); });
